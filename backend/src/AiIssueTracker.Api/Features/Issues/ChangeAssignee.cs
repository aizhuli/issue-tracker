...
public static class ChangeAssignee
{
    public class Endpoint : IEndpoint
    {
        public void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapPatch("/api/projects/{slug}/issues/{number}/assignee", async (
                    string slug,
                    int number,
                    [FromBody] Body body,
                    ISender sender,
                ...
                {
                    var response = await sender.Send(new Request(slug, number, body.AssigneeId), ct);
                    return Results.Ok(response);
                })
                .RequireAuthorization(AuthPolicies.RequireUser)
                .WithTags("Issues")
                .WithName("ChangeAssignee");
        }

        public record Body(string? AssigneeId);
    }

    public record Request(string Slug, int Number, string? AssigneeId) : IRequest<IssueFull>;
    ...
    public class RequestValidator : AbstractValidator<Request>
    {
        public RequestValidator()
        {
            RuleFor(x => x.AssigneeId)
                .Must(id => IdEncoding.TryDecode(id, out _))
    ...
    public class RequestHandler(AppDbContext db) : IRequestHandler<Request, IssueFull>
    {
        public async Task<IssueFull> Handle(Request request, CancellationToken ct)
        {
            ...
            {
                IdEncoding.TryDecode(request.AssigneeId, out var decodedAssigneeId);
                ...
                issue.AssigneeId = decodedAssigneeId;
            }
            else
            {
            ...
            await db.Entry(issue).Reference(i => i.Assignee).LoadAsync(ct);
